...
Dim Shared As Integer num_boxes=220 ' en este ejemplo usamos 220 cajas
dim shared as integer boxes(num_boxes-1)
	...
	ep_world_set_settings(world, 1/4, 20, 10, 0.1, 0.5, 0.00, 0.5, 1) ' dos formas de gravedad
	'ep_world_set_settings(world, 1/4, 20,  0, 0.1, 0.5, 0.04, 0.5, 1) ' ver la diferencia probando ambas
   ...
   ' 220 cajas
   ' se crean desde el "body 3", por que el 0,1 y 2 ya estan ocupados (ver notas por arriba)
	b=0
	For i=0 To 10
		for j=0 To (10 + i * 2)-1
			body = ep_body_create_dynamic(world, FALSE)
			shape = ep_shape_create_box(world, body, 19, 19, 0, 0, 0, 1)
			ep_shape_set_collision(world, body, shape, 1, 1, 0)
			ep_shape_set_material(world, body, shape, 0.2, 0.5, 0, 0)
			ep_body_calculate_mass(world, body)
			ep_body_set_position(world, body, CentroX+(i-5)*70, CentroY+300-32-10-j*20, degtorad(0))
			ep_body_set_gravity(world, body, 0, 0.2)
			boxes(b)=body ' el primer body aqui vale "3", el resto consecutivos
			b+=1
		next
	Next
...
while TRUE
	...
	ScreenLock()
		...
		for b=0 To num_boxes-1 ' 0 a 219, 220 cajas, el primer "body" es el numero 3
